...
import wx.aui as aui # Tab notebooks
...
class MainNotebook(aui.AuiNotebook):
    def __init__(self,parent):
        aui.AuiNotebook.__init__(self,parent, id=wx.ID_ANY, style = aui.AUI_NB_DEFAULT_STYLE)
...
class TreeNotebook():
    def __init__(self,parent):
        #aui.AuiPaneInfo.__init__(self,parent, id=wx.ID_ANY, style = aui.AUI_NB_SCROLL_BUTTONS)
        pane = aui.AuiPaneInfo().Name("pagePane").Gripper(False).CaptionVisible(False).Layer(0).Position(0).CloseButton(False).MaximizeButton(False).Center().Dock()

        #Add first tab to notebook
        self.Tree = wx.TreeCtrl(pane,wx.ID_ANY,wx.DefaultPosition,(200,200),wx.TR_HIDE_ROOT|wx.TR_HAS_BUTTONS)
...
class MainWindow(wx.Frame):
    ...
    def __init__(self, filename="noname.txt"):
        ...
        self.fileTree = TreeNotebook(self)
        ...
        layout = wx.BoxSizer(wx.HORIZONTAL)
        layout_tree = wx.BoxSizer(wx.HORIZONTAL)
        layout_tree.Add(self.fileTree, proportion=1,border=1,flag=wx.EXPAND)
        layout.Add(layout_tree, proportion=1,border=1,flag=wx.ALL|wx.EXPAND)
