...
class TabTree(wx.Panel):
    ...
    def CreateTree(self):
        self.Tree = wx.TreeCtrl(self,wx.ID_ANY,wx.DefaultPosition,(200,200),wx.TR_HIDE_ROOT|wx.TR_HAS_BUTTONS)
        self.root = self.Tree.AddRoot('FileRoot')
        self.UnsortedFiles = self.Tree.AppendItem(self.root,'Unsorted')

        #----
        layout = wx.BoxSizer(wx.HORIZONTAL)
        layout.Add(self.Tree, proportion=1,border=0,flag=wx.ALL|wx.EXPAND)
...
class TreeNotebook(aui.AuiNotebook):
    def __init__(self,parent):
        aui.AuiNotebook.__init__(self,parent, id=wx.ID_ANY, style = aui.AUI_NB_SCROLL_BUTTONS)
        #Add first tab to notebook
        newTab = TabTree(self)
        self.AddPage(newTab, "New Tab")
